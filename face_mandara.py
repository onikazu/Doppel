...
datas = {}
with open('sample_data.pickle', mode='rb') as f:
    datas = pickle.load(f)
# databese配列の作成
face_image_names = []
face_vectors = []
            ...
            try:
                for i in range(len(similar_paths_manager)):
                    ...
                    for j in range(len(similar_paths_manager[i])):
                        images.append(Image.open("./sample_database/{}".format(similar_paths_manager[i][j])))
            ...
            while True:
                ...
                for i in range(len(similar_windows)):
                    ...
                    for j in reversed(range(len(similar_windows[i]))):
                        ...
                        # もしそれぞれの顔の一番の類似顔がadd由来のものならば
                        print(similar_paths_manager)
                        if similar_paths_manager:
                            if similar_paths_manager[i][0] in "_":
                                print("in the add_database")
                                bottom = face_frames[i].bottom
                                left = face_frames[i].left
                                path = similar_paths_manager[i][0]
                                name = path.split("/")[-1].split("_")[0]
                                draw.text((int(left), int(bottom)), name, font=font, fill=(255,255,255,128))
